...
import { VaultStorage } from '../types';
...
const SovereignVault: React.FC = () => {
  ...
  const refreshHeartbeat = async () => {
    ...
    try {
      ...
      setHeartbeat({ status: 'error', message: e.message || 'System standby.' });
    }
  };
  ...
  const restoreSheet = (sheet: any) => {
    ...
    try {
      ...
      alert("Wrap Sheet restored to the Forge.");
  ...
  const handleExportForChatty = () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `Sovereign-Vault-Export-${Date.now()}.json`;
      a.click();
    ...
    <div className="bg-[#050505] min-h-screen text-white pb-32 pt-24 font-sans relative">
      ...
      <section className="max-w-7xl mx-auto px-6 py-12 border-b border-white/5 flex flex-col md:flex-row justify-between items-end relative z-10">
        ...
        <div className="pb-4 flex items-center gap-4">
           ...
           <span className="text-[10px] font-black uppercase tracking-widest text-gray-500">Forge Link: {(heartbeat.status || 'OFFLINE').toUpperCase()}</span>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-6 py-12 flex flex-col md:flex-row gap-12 relative z-10">
        <aside className="w-full md:w-80 space-y-2 shrink-0">
          <button onClick={() => setActiveFolder('sheets')} className={`w-full text-left p-6 transition-all border-l-2 ${activeFolder === 'sheets' ? 'bg-[var(--accent)]/10 border-[var(--accent)] text-[var(--accent)]' : 'bg-black border border-white/5 border-l-transparent text-gray-700'}`}>
            <span className="text-[11px] font-black uppercase tracking-[0.2em]">Wrap Sheets</span>
          </button>
          <button onClick={() => setActiveFolder('books')} className={`w-full text-left p-6 transition-all border-l-2 ${activeFolder === 'books' ? 'bg-[var(--accent)]/10 border-[var(--accent)] text-[var(--accent)]' : 'bg-black border border-white/5 border-l-transparent text-gray-700'}`}>
            <span className="text-[11px] font-black uppercase tracking-[0.2em]">Published Works</span>
          </button>
          <button onClick={() => setActiveFolder('diagnostic')} className={`w-full text-left p-6 transition-all border-l-2 ${activeFolder === 'diagnostic' ? 'bg-cyan-500/10 border-cyan-500 text-cyan-500' : 'bg-black border border-white/5 border-l-transparent text-gray-700'}`}>
            <span className="text-[11px] font-black uppercase tracking-[0.2em]">System Audit</span>
          </button>
          <button onClick={() => setActiveFolder('heartbeat')} className={`w-full text-left p-6 transition-all border-l-2 ${activeFolder === 'heartbeat' ? 'bg-green-500/10 border-green-500 text-green-500' : 'bg-black border border-white/5 border-l-transparent text-gray-700'}`}>
            <span className="text-[11px] font-black uppercase tracking-[0.2em]">Link Health</span>
          </button>
          
          <div className="pt-10">
            <button onClick={handleExportForChatty} disabled={isExporting} className={`w-full text-left p-8 bg-orange-600/10 border border-orange-600/30 text-orange-500 hover:bg-orange-600 hover:text-white transition-all rounded-sm group ${isExporting ? 'animate-pulse opacity-50' : ''}`}>
              <p className="text-[10px] font-black uppercase tracking-widest mb-1">Export Vault</p>
              <p className="text-[8px] font-bold uppercase tracking-widest opacity-60 group-hover:opacity-100">Synchronized Archive Package</p>
            </button>
          </div>
        </aside>
          ...
          {activeFolder === 'sheets' && (
            <div className="grid gap-4 animate-fade-in">
              {vault.sheets.length === 0 ? (
                <div className="p-32 text-center border border-dashed border-white/5 italic text-gray-800 font-serif">Vault Empty. Start forging to populate.</div>
              ) : (
                vault.sheets.map((s: any) => (
                  <div key={s.id} className="p-10 bg-[#0d0d0d] border border-white/5 flex items-center justify-between group rounded-sm hover:border-[var(--accent)]/30 transition-all">
                    <div className="space-y-1">
                       <h3 className="text-3xl font-serif italic text-white tracking-tighter">{s.data?.title || "Untitled Wrap Sheet"}</h3>
                    ...
                    <button onClick={() => restoreSheet(s)} className="px-10 py-4 border border-[var(--accent)] text-[var(--accent)] text-[10px] font-black uppercase tracking-widest hover:bg-[var(--accent)] hover:text-white transition-all rounded-sm">Restore to Forge</button>
                  </div>
                ))
              )}
            </div>
